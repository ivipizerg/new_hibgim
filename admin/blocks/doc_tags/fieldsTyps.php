	...
	class doc_tags_fieldsTyps extends default_tree_fieldsTyps{
		...
		function tags_selector($Field,$Data){
			...
			if($Data!=NULL){
				...
				$D = implode(",",$D);	
				$R = Oxs::G("DBLIB.IDE")->DB()->Exec( "SELECT * FROM `oxs:sql` WHERE `id` != '1' AND `id` IN (".$D.") ORDER BY `position` ASC " , $table_name  );				
				if($R){
					$T = "";
					for($i=0;$i<count($R);$i++){
					   	$T.= "<div value = \"" . $R[$i]["id"] . "\" class=\"oxs_doc_tags_fieldsTyps_add_list_item_selected oxs_doc_tags_fieldsTyps_add_list_item_selected_" . $prefix . "\">" . $R[$i][$field_name] .  " ❌</div>";
					} 
				}				
			}	
			/////////////////////////////////////////////////////////////	
			...
			Oxs::G("oxs_obj")->G( "doc_tags.js:tag_selector", array( array( "prefix" => $prefix , "data" => Oxs::G("crypto.base64")->E($T) ,  "field_name" => $field_name , "table_name" => $table_name , "search_fields" => $search_fields ) ) , "oxs_doc_tags_fieldsTyps_".$prefix );		
			...
			return $Field["description"]."<div class='oxs_doc_tags_fieldsTyps oxs_doc_tags_fieldsTyps_".$prefix."'> ".(
				field::Text("edit_field_".$Field["system_name"],NULL,array("attr" => "autocomplete=\"off\"" , "class"=>"form-control ".$Field["setting"]["class"] , "style" => "margin-top:3px;".$Field["field_style"] , "auto_clear" => $Field["form_name"]) ) .
